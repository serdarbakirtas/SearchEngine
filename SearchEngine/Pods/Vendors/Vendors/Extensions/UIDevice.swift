...
public func osVersionLessThanOrEqual(_ version: String) -> Bool {
    ...
    public func ==(lhs: DeviceType, rhs: DeviceType) -> Bool {
        switch (lhs, rhs) {
        case let (.phone(model1), .phone(model2)): return model1 == model2
        case let (.pad(model1), .pad(model2)): return model1 == model2
        case let (.pod(model1), .pod(model2)): return model1 == model2
        case let (.tv(model1), .tv(model2)): return model1 == model2
