...
extension UIFont {
    ...
    public convenience init(fontName: Font, size: CGFloat) {
    ...
    public class func printFonts() {
        ...
        for familyName in fontFamilyNames {
            trace("------------------------------")
            trace("Font Family Name = [\(familyName)]")
            let names = UIFont.fontNames(forFamilyName: familyName)
            trace("Font Names = [\(names)]")
        }
    }
    ...
    /// - Parameters:
    ///   - width: Max width to fit text.
    ///   - font: Font used in tex̊t, from FontName enum.
    ///   - size: Font size.
    ///   - text: Text.
    /// - Returns: Returns calculated height of string within width using given font
    public static func calculateHeight(width: CGFloat, name: String, fontSize: CGFloat, text: String) -> CGFloat {
    ...
    guard !CTFontManagerRegisterGraphicsFont(font, &error) else {
        ...
        if let errorRef = error?.takeRetainedValue() {
            ...
            print("Failed to load font: \(errorDescription ?? "" as CFString) (\(name).\(fileExtension.type))")
        }
